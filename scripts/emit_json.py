...
def _slow_envelope(n_samples, fs, rel_fluct, rng, chunk_sec=0.5):
    """Multiplicative jitter around 1.0, piecewise-constant over chunk_sec —
    gives windows genuine amplitude variability instead of a pure sinusoid,
    so a per-subject baseline sigma is meaningfully nonzero (real EEG has
    this naturally; deterministic synthetic sinusoids don't)."""
    chunk_samples = int(chunk_sec * fs)
    n_chunks = n_samples // chunk_samples + 2
    coarse = np.clip(rng.normal(1.0, rel_fluct, n_chunks), 0.3, 2.0)
    return np.repeat(coarse, chunk_samples)[:n_samples]


def _synthetic_eeg(fs, duration_s, n_channels, alpha_amp, delta_amp, seed):
    rng = np.random.default_rng(seed)
    n_samples = int(fs * duration_s)
    t = np.arange(n_samples) / fs
    data = np.zeros((n_channels, n_samples))
    for ch in range(n_channels):
        chan_jitter = rng.uniform(0.8, 1.2)
        alpha_env = alpha_amp * chan_jitter * _slow_envelope(n_samples, fs, 1.0, rng)
        delta_env = delta_amp * chan_jitter * _slow_envelope(n_samples, fs, 1.0, rng)
        signal = (
            alpha_env * np.sin(2 * np.pi * 10 * t + rng.uniform(0, 2 * np.pi))
            + delta_env * np.sin(2 * np.pi * 2 * t + rng.uniform(0, 2 * np.pi))
        )
        data[ch] = signal + rng.normal(0, 1.0, n_samples)
    return data


# (alpha_amp, delta_amp) per condition -- descending alpha, rising delta as
# sedation deepens. S01-moderate deliberately reuses S00-moderate's mix: per
# Gaskell et al. (PRD §1), the alpha-delta pattern is present in responders
# too, so SDP should NOT distinguish these two patients even though only one
# of them is behaviorally responsive.
CONDITION_MIX = {
    "baseline": (2.4, 1.0),
    "mild": (2.0, 1.4),
    "moderate": (1.3, 2.1),
...
def main():
    ...
    s00_baseline = _synthetic_eeg(fs, duration_s, len(CH_NAMES), *CONDITION_MIX["baseline"], seed=10)
    for condition, mix in CONDITION_MIX.items():
        cond_data = _synthetic_eeg(fs, duration_s, len(CH_NAMES), *mix, seed=hash(("S00", condition)) & 0xFFFF)
        emit("S00", condition, False, DRUG_CONCENTRATION[condition], s00_baseline, cond_data, fs, duration_s)

    s01_baseline = _synthetic_eeg(fs, duration_s, len(CH_NAMES), *CONDITION_MIX["baseline"], seed=20)
    s01_moderate = _synthetic_eeg(fs, duration_s, len(CH_NAMES), *CONDITION_MIX["moderate"], seed=21)
