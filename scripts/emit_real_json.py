...
# sdp.py's default MIDPOINT/SCALE were fitted on its synthetic self-test, where
# the baseline-to-sedated shift in log10(alpha/delta) is far larger than
# anything real EEG produces. Applied unchanged here they compressed the entire
# cohort into ~61-97, with baseline (86.8 median) and moderate (80.0 median)
# under 7 points apart -- a 0-100 readout using a seventh of its range.
#
# These are fitted to the observed effect size on this dataset: the recovered
# per-subject (r - mu) spans about -1.15 to +0.31 log10 units across baseline
# and moderate, and these constants map that span onto roughly 5-95. Result:
# baseline lands 73-87 (tight, as it should be -- awake subjects all read high)
# while moderate spreads 5-95, which is where anesthetic susceptibility
# actually varies. Refit these if the recording set changes.
REAL_MIDPOINT = 0.420
REAL_SCALE = 0.249
...
def emit(subject, condition, responsive, baseline_data, condition_data, native_ch_names, fs):
    ...
    _, sdp_native = sdp.compute_sdp(baseline_12, condition_12, fs, CH_NAMES, SDP_CHANNELS,
                                    midpoint=REAL_MIDPOINT, scale=REAL_SCALE)
